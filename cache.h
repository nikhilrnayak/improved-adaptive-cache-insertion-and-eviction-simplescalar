...
enum cache_policy {
  ...
  LIP,		/* replace the least recently used block at the lru position */
  BIP,		/* replace bimodally, by chucking the LRU block and placing new one at MRU or LRU position */
  DIP       /* replace adaptively based on saturation counter levels */
};

/* saturation counter for BIP */
struct sat_counter{
unsigned int count;
unsigned int half_level;
unsigned int full_level;
};

struct loc_table
{
  unsigned int pc;
  unsigned int temp_loc;
};

struct intermediate
{
  struct loc_table loc[1]; /* var array */
};
...
struct cache_blk_t
{
  ...
  struct loc_table *loc; /* fields for ORL */
...
struct cache_set_t
{
  ...
  struct intermediate *inter; /*holds way table start address. */
...
struct cache_t
{
  ...
  char aux;			/*is auxilary? */
  char o;			/* ORL or not */
  char misshit;			/* whethe last access hit or miss */
  ...
  struct sat_counter *sat_counter_sel; /* policy selection counter */
  unsigned int pol_sat_lvl; /* set as 0 if saturation counter value from cache_ilx_atr_bip->count < cache_ilx_atr_bip->half_level else 1 */
  unsigned int bip_sat_lvl; /* get the pol_sat_lvl from bip atd to this var in the mtd */
